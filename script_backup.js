...
// Fun sticker display function
function showSticker(stickerType) {
    const sticker = malayalamStickers[stickerType];
    if (!sticker) return;
    
    floatingSticker.innerHTML = `
        <div class="sticker-content" style="border-color: ${sticker.color}">
            <p class="sticker-dialogue">${sticker.dialogue}</p>
        </div>
    `;
    
    floatingSticker.classList.add('show');
    
    setTimeout(() => {
        floatingSticker.classList.remove('show');
    }, 3000);
}
...
function updatePaymentSelects() {
    paymentFromSelect.innerHTML = '<option value="">Who is paying?</option>';
    paymentToSelect.innerHTML = '<option value="">To whom?</option>';
    
    members.forEach(member => {
        const optionFrom = document.createElement('option');
        optionFrom.value = member;
        optionFrom.textContent = member;
        paymentFromSelect.appendChild(optionFrom);
        
        const optionTo = document.createElement('option');
        optionTo.value = member;
        optionTo.textContent = member;
        paymentToSelect.appendChild(optionTo);
    });
}
...
function addTransaction(e) {
    ...
    // Show appropriate sticker based on amount
    if (amount < 100) {
        showSticker('smallExpense');
    } else if (amount < 500) {
        showSticker('mediumExpense');
    } else if (amount < 2000) {
        showSticker('largeExpense');
    } else {
        showSticker('massiveExpense');
    }
...
function updateTransactionList() {
    ...
    dateInput.valueAsDate = new Date();ateElement('span');
...
function updateSummary() {
    ...
    balanceEl.textContent = `₹${totalExpense.toFixed(2)}`;
    incomeEl.textContent = `₹${totalExpense.toFixed(2)}`;
    
    if (allSplitEqually && members.length > 0) {
        expenseEl.textContent = `₹${avgPerPerson.toFixed(2)}`;
    } else {
        expenseEl.textContent = 'Varies';
    }
}

// Update member statistics with stickers
function updateMemberStats() {
    ...
    Object.entries(memberBalances).forEach(([member, stat]) => {
        if (stat.balance > maxBalance) {
            maxBalance = stat.balance;
            richestPerson = member;
        }
        if (stat.balance < minBalance) {
            minBalance = stat.balance;
            poorestPerson = member;
        }
    });
    ...
    members.forEach(member => {pan);
        memberStatsEl.appendChild(div);
    });
    
    // Show sticker for richest/poorest
    if (maxBalance > 500) {
        setTimeout(() => showSticker('richPerson'), 500);
    } else if (minBalance < -500) {
        setTimeout(() => showSticker('poorPerson'), 500);
    }
}
...
function calculateSettlements() {
    ...
    if (settlements.length === 0) {
        ...
        settlements.forEach(settlement => {
            ...
            textSpan.textContent = `${settlement.from} → ${settlement.to}`;
...
function downloadPDF() {
    ...
    if (settlements.length === 0) {
        ...
        doc.setFontSize(10);
        settlements.forEach(settlement => {
            doc.text(`${settlement.from} → ${settlement.to}: ₹${settlement.amount.toFixed(2)}`, 20, y);
            y += 7;
        });
        y += 5;
    }
    ...
    transactions.forEach(transaction => {
        ...
        const desc = transaction.description.length > 25 ? transaction.description.substring(0, 25) + '...' : transaction.description;
        doc.text(desc, 45, y);
        doc.text(transaction.paidBy, 100, y);
        doc.text(splitInfo, 130, y);
        doc.text(`₹${transaction.amount.toFixed(2)}`, 165, y);
        y += 7;
    });
