...
const currentTripDescription = document.getElementById('current-trip-description');
...
function loadTrip(tripId) {
    ...
    currentTripDescription.textContent = trip.description || '';
...
function addExpense(e) {
    ...
    if (date === '' || category === '' || description === '' || paidBy === '' || isNaN(amount) || amount <= 0) {
...
function updateExpenseList() {
    ...
    sortedExpenses.forEach(expense => {
        ...
        descSpan.textContent = `${expense.category} - ${expense.description}`;
...
function exportPDF() {
    ...
    trip.expenses.forEach(expense => {
        ...
        const category = expense.category.substring(0, 15);
        const desc = expense.description.length > 20 ? expense.description.substring(0, 20) + '...' : expense.description;
        
        doc.text(formattedDate, 20, y);
        doc.text(category, 45, y);
        doc.text(desc, 80, y);
        doc.text(expense.paidBy, 125, y);
        doc.text(`₹${expense.amount.toFixed(2)}`, 165, y);
        y += 7;
    });
